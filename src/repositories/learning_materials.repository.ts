...
import { LearningMaterialBelongsToDomain, LearningMaterialBelongsToDomainLabel } from "../entities/relationships/LearningMaterialBelongsToDomain";
...
import { attachUniqueNodes, detachUniqueNodes, findOne, attachNodes, detachNodes, getRelatedNodes } from "./util/abstract_graph_repo";
...
export const attachLearningMaterialCoversConcepts = (learningMaterialId: string,
    conceptIds: string[],
    props: { userId: string }): Promise<{ learningMaterial: LearningMaterial, concepts: Concept[] }> =>
    attachNodes<LearningMaterial, LearningMaterialCoversConcept, Concept>({
        ...
        if (items.length !== conceptIds.length) logger.warn('attachLearningMaterialCoversConcepts: some concepts in ' + JSON.stringify(conceptIds) + ' not found');
...
export const detachLearningMaterialCoversConcepts = (learningMaterialId: string, conceptIds: string[]):
