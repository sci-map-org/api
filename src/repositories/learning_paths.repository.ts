...
import {
  ResourceBelongsToLearningPath,
  ResourceBelongsToLearningPathLabel,
} from '../entities/relationships/ResourceBelongsToLearningPath';
...
export const createLearningPath = (userId: string, data: CreateLearningPathData): Promise<LearningPath> =>
  createRelatedNode<User, UserCreatedLearningMaterial, LearningPath>({
    originNode: {
      label: UserLabel,
      filter: { _id: userId },
    },
...
export const findLearningPath = findOne<LearningPath, { _id: string } | { key: string }>({
  label: LearningMaterialLabel,
});

export const findLearningPathCreatedBy = (
  userId: string,
  learningPathFilter: { _id: string } | { key: string }
): Promise<LearningPath | null> =>
  getOptionalRelatedNode<User, UserCreatedLearningMaterial, LearningPath>({
...
export const getLearningPathResourceItems = async (learningPathId: string) => {
  ...
  q.raw(
    `WITH DISTINCT i_rel { .*, resource: properties(i)} as i_item, collect(r_rel { .*, resource: properties(r)}) as r_item`
  );
...
export const updateLearningPath = updateOne<LearningPath, { _id: string }, UpdateLearningPathData>({
  label: LearningPathLabel,
});

export const addResourcesToLearningPath = async (
...
export const deleteLearningPathResourceItems = async (learningPathId: string): Promise<LearningPath | null> => {
  ...
  q.optionalMatch([
    ...
    relation(
      'out',
      'rels',
      [LearningPathStartsWithResourceLabel, ResourceHasNextInLearningPathResourceLabel],
      { learningPathId },
      [1, 100]
    ),
    node('r', ResourceLabel),
  ...
  q.raw(
    `WHERE NOT (r)-[:${LearningPathStartsWithResourceLabel}|${ResourceHasNextInLearningPathResourceLabel} {learningPathId: $learningPathId}]->(:Resource)`,
    { learningPathId }
  );
...
export const attachResourceToLearningPath = async (
  learningPathId: string,
  resourceId: string
): Promise<{ learningPath: LearningPath; resource: Resource }> =>
...
export const detachResourceFromLearningPath = async (
  learningPathId: string,
  resourceId: string
): Promise<{ learningPath: LearningPath; resource: Resource }> =>
  detachUniqueNodes<Resource, ResourceBelongsToLearningPath, LearningPath>({
    originNode: { label: ResourceLabel, filter: { _id: resourceId } },
    relationship: { label: ResourceBelongsToLearningPathLabel, filter: {} },
    destinationNode: { label: LearningPathLabel, filter: { _id: learningPathId } },
  }).then(({ originNode, destinationNode }) => ({ learningPath: destinationNode, resource: originNode }));

export const getLearningPathComplementaryResources = (learningPathId: string): Promise<Resource[]> =>
  getRelatedNodes<LearningPath, ResourceBelongsToLearningPath, Resource>({
    originNode: {
      label: LearningPathLabel,
...
export const attachUserStartedLearningPath = (
  userId: string,
  learningPathId: string
...
export const updateUserStartedLearningPath = (
  userId: string,
  learningPathId: string,
  data: { completedAt?: number | null }
): Promise<{ user: User; relationship: UserStartedLearningPath; learningPath: LearningPath }> =>
  updateUniqueRelationship<User, UserStartedLearningPath, LearningPath>({
    originNode: { label: UserLabel, filter: { _id: userId } },
    relationship: { label: UserStartedLearningPathLabel, updateProps: data },
    destinationNode: { label: LearningPathLabel, filter: { _id: learningPathId } },
  }).then(({ originNode, relationship, destinationNode }) => ({
    user: originNode,
    relationship,
...
export const getUserStartedLearningPath = (
  userId: string,
  learningPathId: string
...
): Promise<{ user: User; relationship: UserStartedLearningPath }[]> =>
